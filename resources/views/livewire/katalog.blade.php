<div>
  ...
  <div class="modal fade" id="modal-pesan-produk" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true"
    wire:ignore.self>
                ...
                <div class="d-flex align-items-center">
                  <div class="input-group input-group-sm w-auto">
                    <button wire:click="tambahJumlahPesanan" type="button" class="btn btn-outline-secondary"
                      id="btn-kurang-jumlah">
                      <i class="bi bi-dash"></i>
                    </button>
                    <input wire:model="jumlahPesanan" type="number" name="jumlah" id="jumlah"
                      class="form-control text-center" value="1" style="max-width: 80px;">
        ...
        <div class="card h-100 border-0 shadow-sm rounded-4 overflow-hidden hover-shadow">
          <div class="position-relative"
            ...
            <img src="{{ asset('storage/' . $item->gambar) }}" class="img-fluid product-image"
              alt="{{ $item->nama_produk }}" style="max-height: 100%; object-fit: contain; transition: transform 0.3s;">
            @if ($item->totalPersediaan() === 0)
              <div
                class="position-absolute w-100 h-100 d-flex align-items-center justify-content-center bg-dark bg-opacity-50">
