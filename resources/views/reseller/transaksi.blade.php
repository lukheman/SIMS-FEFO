        ...
        <div id="datatable_wrapper" class="dataTables_wrapper dt-bootstrap5">
            <div class="row">
                <div class="col-12 col-md-6 mb-2">
                    ...
                    <a href="{{ route('reseller.transaksi', ['belumbayar' => 0] )}}"
                        class="btn btn-sm btn-outline-primary {{ request()->has('belumbayar') || count(request()->all()) === 0 ? 'active' : ''}}"
                        id="btn-belum-bayar">Belum Bayar</a>
                    <a href="{{ route('reseller.transaksi', ['pending' => 1] )}}"
                        class="btn btn-sm btn-outline-primary {{ request()->has('pending') ? 'active' : ''}}"
                        id="btn-pending">Pending</a>
                    <a href="{{ route('reseller.transaksi', ['diproses' => 1]) }}"
                        class="btn btn-sm btn-outline-primary {{ request()->has('diproses') ? 'active' : ''}}"
                        id="btn-diproses">Diproses</a>
                    <a href="{{ route('reseller.transaksi', ['dikirim' => 1])}}"
                        class="btn btn-sm btn-outline-primary {{ request()->has('dikirim') ? 'active' : ''}}"
                        id="btn-dikirim">Dikirim</a>
                    <a href="{{ route('reseller.transaksi', ['selesai' => 1])}}"
                        class="btn btn-sm btn-outline-primary {{ request()->has('selesai') ? 'active' : ''}}"
                        id="btn-selesai">Selesai</a>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($item->metode_pembayaran === \App\Constants\MetodePembayaran::TRANSFER)
                                        ...
                                        @if (isset($item->bukti_pembayaran))
                                            <button type="button"
                                                class="btn btn-sm btn-outline-primary btn-lihat-bukti-pembayaran"
                                                data-id-transaksi="{{ $item->id }}"
                                                data-bukti-pembayaran="{{ $item->bukti_pembayaran }}"
                                                data-bs-toggle="modal" data-bs-target="#modal-lihat-bukti-pembayaran">
                                ...
                                <td>
                                    <button type="button" class="btn btn-sm btn-outline-secondary btn-detail-transaksi"
                                        data-id-transaksi="{{ $item->id }}"
                                        data-bs-toggle="modal" data-bs-target="#modal-detail-transaksi">
...
<div class="modal fade" id="modal-lihat-bukti-pembayaran" style="display: none;" aria-modal="true" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer justify-content-between">
                ...
                <button type="button" class="btn btn-primary" id="btn-ubah-bukti-pembayara" data-bs-toggle="modal" data-bs-target="#modal-kirim-bukti-pembayaran">
                    <i class="fas fa-send"></i> Ubah
                </button>
            </div>
        </div>
        <!-- /.modal-content -->
...
<div class="modal fade" id="modal-kirim-bukti-pembayaran" style="display: none;" aria-modal="true" role="dialog">
